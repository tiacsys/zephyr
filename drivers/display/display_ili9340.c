...
int ili9340_regs_init(const struct device *dev)
{
	...
	r = config->reg_transmit_fn(dev, ILI9340_GAMSET, regs->gamset,
					 ILI9340_GAMSET_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = config->reg_transmit_fn(dev, ILI9340_FRMCTR1, regs->frmctr1,
					 ILI9340_FRMCTR1_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = config->reg_transmit_fn(dev, ILI9340_DISCTRL, regs->disctrl,
					 ILI9340_DISCTRL_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = config->reg_transmit_fn(dev, ILI9340_PWCTRL1, regs->pwctrl1,
					 ILI9340_PWCTRL1_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = config->reg_transmit_fn(dev, ILI9340_PWCTRL2, regs->pwctrl2,
					 ILI9340_PWCTRL2_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = config->reg_transmit_fn(dev, ILI9340_VMCTRL1, regs->vmctrl1,
					 ILI9340_VMCTRL1_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = config->reg_transmit_fn(dev, ILI9340_VMCTRL2, regs->vmctrl2,
					 ILI9340_VMCTRL2_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = config->reg_transmit_fn(dev, ILI9340_PGAMCTRL, regs->pgamctrl,
	...
	r = config->reg_transmit_fn(dev, ILI9340_NGAMCTRL, regs->ngamctrl,
