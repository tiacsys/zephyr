...
int ili9342c_regs_init(const struct device *dev)
{
	...
	r = config->reg_transmit_fn(dev, ILI9342C_SETEXTC, regs->setextc,
	...
	r = config->reg_transmit_fn(dev, ILI9342C_GAMSET, regs->gamset,
					 ILI9342C_GAMSET_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = config->reg_transmit_fn(dev, ILI9342C_IFMODE, regs->ifmode,
					 ILI9342C_IFMODE_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = config->reg_transmit_fn(dev, ILI9342C_FRMCTR1, regs->frmctr1,
	...
	r = config->reg_transmit_fn(dev, ILI9342C_INVTR, regs->invtr,
					 ILI9342C_INVTR_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = config->reg_transmit_fn(dev, ILI9342C_DISCTRL, regs->disctrl,
	...
	r = config->reg_transmit_fn(dev, ILI9342C_ETMOD, regs->etmod,
					 ILI9342C_ETMOD_LEN);
	if (r < 0) {
		return r;
	}
	...
	r = config->reg_transmit_fn(dev, ILI9342C_PWCTRL1, regs->pwctrl1,
	...
	r = config->reg_transmit_fn(dev, ILI9342C_PWCTRL2, regs->pwctrl2,
	...
	r = config->reg_transmit_fn(dev, ILI9342C_PWCTRL3, regs->pwctrl3,
	...
	r = config->reg_transmit_fn(dev, ILI9342C_VMCTRL1, regs->vmctrl1,
	...
	r = config->reg_transmit_fn(dev, ILI9342C_PGAMCTRL, regs->pgamctrl,
	...
	r = config->reg_transmit_fn(dev, ILI9342C_NGAMCTRL, regs->ngamctrl,
	...
	r = config->reg_transmit_fn(dev, ILI9342C_IFCTL, regs->ifctl,
					 ILI9342C_IFCTL_LEN);
	if (r < 0) {
		return r;
	}
