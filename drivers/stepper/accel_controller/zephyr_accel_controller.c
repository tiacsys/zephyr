...
void accel_controller_callback_handler(const struct device *dev, const enum stepper_ctrl_event event,
				       void *user_data)
{
	struct zephyr_accel_controller_data *data = user_data;
	enum stepper_ctrl_event modified_event = event;

	if (event == STEPPER_CTRL_EVENT_STEPS_COMPLETED && data->mode == STEPPER_CTRL_RUN_MODE_STOP) {
...
static int zephyr_accel_controller_move_by(const struct device *dev, const int32_t micro_steps)
{
	...
	if ((data->mode == STEPPER_CTRL_RUN_MODE_POSITION || data->mode == STEPPER_CTRL_RUN_MODE_STOP) &&
	    micro_steps != 0) {
	...
	if (data->steps_s_current < data->steps_s) {
		...
		if (!running) {
			...
			stepper_ctrl_set_microstep_interval(config->stepper_ctrl, target_step_interval);
		}
							  ...
							  const uint64_t microstep_interval_ns)
{
	...
	if (data->mode == STEPPER_CTRL_RUN_MODE_HOLD) {
		return stepper_ctrl_set_microstep_interval(config->stepper_ctrl, microstep_interval_ns);
	}
...
int zephyr_accel_controller_run(const struct device *dev, const enum stepper_ctrl_direction direction)
{
	const struct zephyr_accel_controller_config *config = dev->config;
	struct zephyr_accel_controller_data *data = dev->data;
	bool running = false;
