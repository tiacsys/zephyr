				       ...
				       void *user_data)
{
	...
	enum stepper_event modified_event = event;

	if (event == STEPPER_EVENT_STEPS_COMPLETED && data->mode == STEPPER_RUN_MODE_STOP) {
		modified_event = STEPPER_EVENT_STOPPED;
	}
...
static inline int accel_controller_event_callback_init(const struct device *dev)
{
	...
	if (!data->callback_initialized) {
		ret = stepper_set_event_callback(config->stepper_ctrl,
							  ...
							  const uint64_t microstep_interval_ns)
{
	 ...
	 */
	if (data->mode == STEPPER_RUN_MODE_POSITION) {
	...
	if (data->mode == STEPPER_RUN_MODE_VELOCITY) {
		return stepper_run(dev, data->direction);
	}

	if (data->mode == STEPPER_RUN_MODE_HOLD) {
		return stepper_set_microstep_interval(config->stepper_ctrl, microstep_interval_ns);
	}
...
int zephyr_accel_controller_run(const struct device *dev, const enum stepper_direction direction)
{
	...
	ret = stepper_is_moving(config->stepper_ctrl, &running);
	if (ret != 0) {
