...
// TODO: Possibly extend api: speed reached event
							   ...
							   uint64_t start_steps_s)
{
	...
	return ((double)(start_steps_s * start_steps_s * data->decel_time_microstep_ns) /
		NSEC_PER_SEC / 2) *
						       ...
						       uint32_t *new_steps_s)
{
	...
	*accel_steps -= (uint32_t)round(
		(double)(diff * data->accel_time_microstep_ns) /
		(double)(data->accel_time_microstep_ns + data->decel_time_microstep_ns));
	*decel_steps -= (uint32_t)round(
		(double)(diff * data->decel_time_microstep_ns) /
		(double)(data->accel_time_microstep_ns + data->decel_time_microstep_ns));

	/* If both of the above terms end in .5 before rounding, there is one step less than there
	 * should be. Thus add it back. It could be added to either one, but the decision is to use
	 * the acceleration steps.*/
	if (*accel_steps + *decel_steps < abs(micro_steps)) {
		*accel_steps += 1;
	}

	double accel = NSEC_PER_SEC / (double)data->accel_time_microstep_ns;

	*new_steps_s =
		sqrtl((double)(steps_s_start * steps_s_start) + 2.0 * (*accel_steps) * accel);
	*accel_steps *= config->accel_decel_mult;
	*decel_steps *= config->accel_decel_mult;
							      ...
							      uint32_t stop_steps_s)
{
	...
	if (start_steps_s <= stop_steps_s) {
		return (stop_steps_s - start_steps_s) * data->accel_time_microstep_ns /
		       config->update_interval_ns;
	} else {
		return (uint32_t)abs((int32_t)start_steps_s - (int32_t)stop_steps_s) *
		       data->decel_time_microstep_ns / config->update_interval_ns;
	}
}
...
static int zephyr_accel_controller_move_by(const struct device *dev, const int32_t micro_steps)
{
	...
	if (data->shadow_steps_s >= steps_s_start) {
		accel_steps = (abs((int32_t)data->shadow_steps_s - (int32_t)steps_s_start) *
			       ...
			       (double)data->accel_time_microstep_ns / NSEC_PER_SEC / 2) *
				      config->accel_decel_mult +
			      (base_steps_s *
			       abs((int32_t)data->shadow_steps_s - (int32_t)steps_s_start) *
			       (double)data->accel_time_microstep_ns / NSEC_PER_SEC) *
				      config->accel_decel_mult;
	} else {
		accel_steps = (abs((int32_t)data->shadow_steps_s - (int32_t)steps_s_start) *
			       abs((int32_t)data->shadow_steps_s - (int32_t)steps_s_start) *
			       (double)data->decel_time_microstep_ns / NSEC_PER_SEC / 2) *
				      config->accel_decel_mult +
			      (base_steps_s *
			       abs((int32_t)data->shadow_steps_s - (int32_t)steps_s_start) *
			       (double)data->decel_time_microstep_ns / NSEC_PER_SEC) *
				      config->accel_decel_mult;
	}
	...
	data->stop_segments =
		(data->steps_s) * data->decel_time_microstep_ns / (config->update_interval_ns);
...
void zephyr_accel_controller_wq_accel_handler(struct k_work *work)
{
	...
	uint32_t target_steps_s = ((uint64_t)data->segment_index * 1000 + 500) *
					  (data->steps_s - data->steps_s_start) /
...
void zephyr_accel_controller_wq_decel_start_handler(struct k_work *work)
{
	...
	uint32_t target_steps_s = ((uint64_t)data->segment_index * 1000 + 500) *
					  (data->steps_s_start - data->steps_s) /
					  (data->start_segments * 1000) +
				  data->steps_s;
	uint64_t target_step_interval = NSEC_PER_SEC / target_steps_s;
	/* Clamp new microstep interval */
...
void zephyr_accel_controller_wq_decel_handler(struct k_work *work)
{
	...
	uint32_t target_steps_s = ((uint64_t)data->segment_index * 1000 + 500) *
				  data->steps_s_stop_start / (data->stop_segments * 1000);
	if (target_steps_s == 0) {
		target_steps_s = 1;
	}
	uint64_t target_step_interval = NSEC_PER_SEC / target_steps_s;
...
int zephyr_accel_controller_stop(const struct device *dev)
{
	...
	data->stop_segments =
		data->steps_s_current * data->decel_time_microstep_ns / config->update_interval_ns;
	data->segment_index = data->stop_segments;
...
#define ZEPHYR_ACCEL_CONTROLLER_DEFINE(inst)                                                       \
	static const struct zephyr_accel_controller_config zephyr_accel_controller_config_##inst = \
		{                                                                                  \
			...
			.accel_decel_mult = (double)DT_INST_PROP(inst, step_adjustment) / 1000.0,  \
	};                                                                                         \
