...
#ifndef ZEPHYR_DRIVER_STEPPER_CONTROLLER_EVENT_COMMON_H_
#define ZEPHYR_DRIVER_STEPPER_CONTROLLER_EVENT_COMMON_H_

/**
 * @brief Stepper Motion Controller APIs
 * @defgroup common Stepper Motion Controller APIs
 ...
 */
struct stepper_controller_event_common_data {
	...
	uint8_t event_msgq_buffer[CONFIG_STEPPER_CONTROLLER_EVENT_QUEUE_LEN *
 ...
 */
#define STEPPER_CONTROLLER_EVENT_COMMON_DT_DATA_INIT(node_id)                                      \
	{                                                                                          \
 ...
 * @param inst Instance.
 */
#define STEPPER_CONTROLLER_EVENT_COMMON_DT_INST_DATA_INIT(inst)                                    \
	STEPPER_CONTROLLER_EVENT_COMMON_DT_DATA_INIT(DT_DRV_INST(inst))
...
/**
 * @brief Trigger callback function for stepper_ctrl events.
 * @param dev Pointer to the device structure.
 * @param event The stepper_event to trigger the callback for.
 */
void stepper_controller_event_common_trigger_callback(const struct device *dev,
						      enum stepper_event event);

/**
 * @brief Set a callback function for stepper_ctrl events.
 *
 * This function sets a user-defined callback that will be invoked when a stepper_ctrlevent
 * occurs.
 *
 * @param dev Pointer to the device structure.
 ...
 */
static inline int stepper_controller_event_common_set_event_callback(
	const struct device *dev, stepper_event_callback_t callback, void *user_data)
{
...
#endif /* ZEPHYR_DRIVER_STEPPER_CONTROLLER_EVENT_COMMON_H_ */
