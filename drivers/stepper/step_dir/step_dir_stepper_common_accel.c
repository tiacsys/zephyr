...
static inline int step_dir_stepper_accel_perform_step(const struct device *dev)
{
	...
	if (data->step_index == data->accel_steps && data->acceleration != 0) {
		...
		data->timing_data.microstep_interval_ns = 0;
		config->timing_source->update(dev, &data->timing_data);
	}
...
int step_dir_stepper_common_accel_positioning(const struct device *dev, int32_t micro_steps)
{
	...
	data->microstep_interval_ns = data->new_interval;
	...
	if (data->current_interval == data->microstep_interval_ns &&
							 ...
							 const uint64_t microstep_interval_ns)
{
	...
	K_SPINLOCK(&data->lock) {
		data->new_interval = microstep_interval_ns;
		data->timing_data.microstep_interval_ns = data->microstep_interval_ns;
	}
				      ...
				      const enum stepper_direction direction)
{
	...
	int ret = step_dir_stepper_accel_calculate_acceleration(dev, 0, data->microstep_interval_ns,
								true);
	if (ret != 0) {
		return ret;
	}
	...
	K_SPINLOCK(&data->lock) {
		...
		if (data->direction == direction || !config->timing_source->is_running(dev) || data->acceleration == 0) {
			...
			if (data->direction == STEPPER_DIRECTION_POSITIVE &&
			    data->microstep_interval_ns == 0) {
				data->step_count = steps / 2;
			} else if (data->microstep_interval_ns == 0) {
				data->step_count = -steps / 2;
			}
