...
{
	int target_position = STEPPER_STEPS_0_TO_LOW + STEPPER_STEPS_LOW_1_SEC / 10;
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_LOW_SPEED_INTERVAL));
	zassert_ok(stepper_run(fixture->dev, STEPPER_DIRECTION_POSITIVE));
...
ZTEST_F(accel_controller, test_run_positive_direction_from_higher_speed)
{
	int target_position = STEPPER_STEPS_0_TO_LOW + STEPPER_STEPS_LOW_TO_HIGH +
			      STEPPER_STEPS_HIGH_1_SEC / 10 + STEPPER_STEPS_LOW_TO_HIGH +
			      STEPPER_STEPS_LOW_1_SEC / 10;
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_HIGH_SPEED_INTERVAL));
	zassert_ok(stepper_run(fixture->dev, STEPPER_DIRECTION_POSITIVE));
	k_msleep(STEPPER_TIMEOUT_0_TO_LOW_MS + STEPPER_TIMEOUT_LOW_TO_HIGH_MS + 100);
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_LOW_SPEED_INTERVAL));
	k_msleep(STEPPER_TIMEOUT_LOW_TO_HIGH_MS + 100);
...
{
	int target_position = STEPPER_STEPS_0_TO_LOW + STEPPER_STEPS_LOW_1_SEC / 10 +
			      STEPPER_STEPS_LOW_TO_HIGH + STEPPER_STEPS_HIGH_1_SEC / 10;
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_LOW_SPEED_INTERVAL));
	zassert_ok(stepper_run(fixture->dev, STEPPER_DIRECTION_POSITIVE));
	k_msleep(STEPPER_TIMEOUT_0_TO_LOW_MS + 100);
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_HIGH_SPEED_INTERVAL));
...
{
	int target_position = STEPPER_STEPS_0_TO_LOW_LOWER_HALF;
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_HIGH_SPEED_INTERVAL));
	zassert_ok(stepper_run(fixture->dev, STEPPER_DIRECTION_POSITIVE));
	k_msleep(STEPPER_TIMEOUT_0_TO_LOW_HALF_MS / 2);
...
ZTEST_F(accel_controller, test_stop_positive_direction)
{
	...
	POLL_AND_CHECK_SIGNAL(
		stepper_signal, stepper_event, STEPPER_EVENT_STOPPED,
		K_MSEC(STEPPER_TIMEOUT_0_TO_LOW_MS + CONFIG_STEPPER_TEST_TIMEOUT_TOLERANCE_MS));
	bool moving = true;
	zassert_ok(stepper_is_moving(fixture->dev, &moving));
	zassert_false(moving, "Stepper should not be moving, but is");
	int position1;
...
ZTEST_F(accel_controller, test_move_by_positive_direction)
{
	...
	POLL_AND_CHECK_SIGNAL(stepper_signal, stepper_event, STEPPER_EVENT_STEPS_COMPLETED,
			      K_MSEC(2333 + CONFIG_STEPPER_TEST_TIMEOUT_TOLERANCE_MS));
...
ZTEST_F(accel_controller, test_move_by_positive_direction_insufficient_steps_for_full_speed)
{
	...
	POLL_AND_CHECK_SIGNAL(stepper_signal, stepper_event, STEPPER_EVENT_STEPS_COMPLETED,
			      K_MSEC(STEPPER_TIMEOUT_0_TO_LOW_MS + STEPPER_TIMEOUT_0_TO_LOW_MS +
				     CONFIG_STEPPER_TEST_TIMEOUT_TOLERANCE_MS));
	...
	test_move_by_positive_direction_insufficient_steps_for_full_speed_while_moving_higher_speed)
{
	...
	POLL_AND_CHECK_SIGNAL(
		...
		K_MSEC(STEPPER_TIMEOUT_LOW_TO_HIGH_HALF_MS + STEPPER_TIMEOUT_LOW_TO_HIGH_HALF_MS +
		       STEPPER_TIMEOUT_0_TO_LOW_MS + CONFIG_STEPPER_TEST_TIMEOUT_TOLERANCE_MS));
	STEPPER_POSITION_COMPARE_POSITIVE(fixture, target_position);
}
...
ZTEST_F(accel_controller, test_move_by_changing_microstep_interval_fails)
{
	...
	k_msleep(1500);
	POLL_AND_CHECK_SIGNAL(stepper_signal, stepper_event, STEPPER_EVENT_STEPS_COMPLETED,
			      K_MSEC(500 + CONFIG_STEPPER_TEST_TIMEOUT_TOLERANCE_MS));
...
ZTEST_F(accel_controller, test_move_by_0_steps_immediate_stop_positive_direction)
{
	...
	POLL_AND_CHECK_SIGNAL(stepper_signal, stepper_event, STEPPER_EVENT_STEPS_COMPLETED,
			      K_MSEC(CONFIG_STEPPER_TEST_TIMEOUT_TOLERANCE_MS));
	bool moving = true;
	zassert_ok(stepper_is_moving(fixture->dev, &moving));
...
ZTEST_F(accel_controller, test_move_by_fails_while_moving_positioning_mode)
{
	...
	POLL_AND_CHECK_SIGNAL(stepper_signal, stepper_event, STEPPER_EVENT_STEPS_COMPLETED,
			      K_MSEC(STEPPER_STEPS_LOW_1_SEC / 2 + STEPPER_TIMEOUT_0_TO_LOW_MS +
				     CONFIG_STEPPER_TEST_TIMEOUT_TOLERANCE_MS));
...
ZTEST_F(accel_controller, test_move_by_to_run_negative_direction)
{
	...
	zassert_ok(stepper_move_by(
		fixture->dev,
		-(STEPPER_STEPS_0_TO_LOW + STEPPER_STEPS_LOW_1_SEC / 10 + STEPPER_STEPS_0_TO_LOW)));
	k_msleep(STEPPER_TIMEOUT_0_TO_LOW_MS);
...
ZTEST_F(accel_controller, test_move_to_fails_while_moving_velocity_mode)
{
	...
	zassert_ok(stepper_run(fixture->dev, STEPPER_DIRECTION_POSITIVE));
	k_msleep(STEPPER_TIMEOUT_0_TO_LOW_MS);
	int ret = stepper_move_to(
		fixture->dev,
		STEPPER_STEPS_0_TO_LOW +
			STEPPER_STEPS_LOW_1_SEC *
				((double)CONFIG_STEPPER_TEST_STEPPING_TOLERANCE / 100.0) +
			STEPPER_STEPS_0_TO_LOW);
...
ZTEST_F(accel_controller, test_move_to_fails_while_moving_positioning_mode)
{
	...
	POLL_AND_CHECK_SIGNAL(stepper_signal, stepper_event, STEPPER_EVENT_STEPS_COMPLETED,
			      K_MSEC(500 + STEPPER_TIMEOUT_0_TO_LOW_MS +
				     CONFIG_STEPPER_TEST_TIMEOUT_TOLERANCE_MS));
...
{
	int target_position =
		STEPPER_STEPS_0_TO_LOW + STEPPER_STEPS_LOW_TO_HIGH + STEPPER_STEPS_HIGH_1_SEC / 10;
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_LOW_SPEED_INTERVAL));
	zassert_ok(stepper_run(fixture->dev, STEPPER_DIRECTION_POSITIVE));
	k_msleep(STEPPER_TIMEOUT_0_TO_LOW_MS / 2);
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_HIGH_SPEED_INTERVAL));
	k_msleep(STEPPER_TIMEOUT_0_TO_LOW_MS / 2 + STEPPER_TIMEOUT_LOW_TO_HIGH_MS + 100);
...
{
	int target_position = STEPPER_STEPS_0_TO_LOW + STEPPER_STEPS_LOW_TO_HIGH_LOWER_HALF +
			      STEPPER_STEPS_LOW_TO_HIGH_LOWER_HALF + STEPPER_STEPS_LOW_1_SEC / 10;
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_HIGH_SPEED_INTERVAL));
	zassert_ok(stepper_run(fixture->dev, STEPPER_DIRECTION_POSITIVE));
	k_msleep(STEPPER_TIMEOUT_0_TO_LOW_MS + STEPPER_TIMEOUT_LOW_TO_HIGH_HALF_MS);
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_LOW_SPEED_INTERVAL));
	k_msleep(STEPPER_TIMEOUT_LOW_TO_HIGH_HALF_MS + 100);
...
{
	int target_position =
		-(STEPPER_STEPS_0_TO_LOW + STEPPER_STEPS_LOW_TO_HIGH_LOWER_HALF +
		  STEPPER_STEPS_LOW_TO_HIGH_LOWER_HALF + STEPPER_STEPS_LOW_1_SEC / 10);
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_HIGH_SPEED_INTERVAL));
	zassert_ok(stepper_run(fixture->dev, STEPPER_DIRECTION_NEGATIVE));
	k_msleep(STEPPER_TIMEOUT_0_TO_LOW_MS + STEPPER_TIMEOUT_LOW_TO_HIGH_HALF_MS);
...
ZTEST_F(accel_controller, test_run_deceleration_to_acceleration_positive_direction)
{
	int target_position = STEPPER_STEPS_0_TO_LOW + STEPPER_STEPS_LOW_TO_HIGH +
			      ...
			      STEPPER_STEPS_LOW_TO_HIGH_UPPER_HALF + STEPPER_STEPS_HIGH_1_SEC / 10;
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_HIGH_SPEED_INTERVAL));
	zassert_ok(stepper_run(fixture->dev, STEPPER_DIRECTION_POSITIVE));
...
ZTEST_F(accel_controller, test_run_deceleration_to_acceleration_negative_direction)
{
	int target_position =
		-(STEPPER_STEPS_0_TO_LOW + STEPPER_STEPS_LOW_TO_HIGH +
		  STEPPER_STEPS_LOW_TO_HIGH_UPPER_HALF + STEPPER_STEPS_LOW_TO_HIGH_UPPER_HALF +
		  STEPPER_STEPS_HIGH_1_SEC / 10);
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_HIGH_SPEED_INTERVAL));
	zassert_ok(stepper_run(fixture->dev, STEPPER_DIRECTION_NEGATIVE));
	k_msleep(STEPPER_TIMEOUT_0_TO_LOW_MS + STEPPER_TIMEOUT_LOW_TO_HIGH_MS);
...
ZTEST_F(accel_controller, test_run_deceleration_to_deceleration_positive_direction)
{
	...
	k_msleep(STEPPER_TIMEOUT_0_TO_LOW_HALF_MS + 200);
...
ZTEST_F(accel_controller, test_run_deceleration_to_deceleration_negative_direction)
{
	int target_position =
		-(STEPPER_STEPS_0_TO_LOW + STEPPER_STEPS_LOW_TO_HIGH + STEPPER_STEPS_LOW_TO_HIGH +
		  STEPPER_STEPS_0_TO_LOW_UPPER_HALF + STEPPER_STEPS_LOW_1_SEC / 10);
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_HIGH_SPEED_INTERVAL));
	zassert_ok(stepper_run(fixture->dev, STEPPER_DIRECTION_NEGATIVE));
	k_msleep(STEPPER_TIMEOUT_0_TO_LOW_MS + STEPPER_TIMEOUT_LOW_TO_HIGH_MS);
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_LOW_SPEED_INTERVAL));
...
ZTEST_F(accel_controller, test_move_by_acceleration_to_acceleration_positive_direction)
{
	...
	POLL_AND_CHECK_SIGNAL(stepper_signal, stepper_event, STEPPER_EVENT_STEPS_COMPLETED,
			      K_MSEC(STEPPER_TIMEOUT_LOW_TO_HIGH_MS +
				     ...
				     390 + CONFIG_STEPPER_TEST_TIMEOUT_TOLERANCE_MS));
...
{
	int target_position = STEPPER_STEPS_0_TO_LOW + STEPPER_STEPS_LOW_TO_HIGH_LOWER_HALF + 1500;
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_HIGH_SPEED_INTERVAL));
	zassert_ok(stepper_run(fixture->dev, STEPPER_DIRECTION_POSITIVE));
	k_msleep(STEPPER_TIMEOUT_0_TO_LOW_MS + STEPPER_TIMEOUT_LOW_TO_HIGH_HALF_MS);
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_LOW_SPEED_INTERVAL));
	...
	POLL_AND_CHECK_SIGNAL(stepper_signal, stepper_event, STEPPER_EVENT_STEPS_COMPLETED,
			      K_MSEC(STEPPER_TIMEOUT_LOW_TO_HIGH_HALF_MS +
				     STEPPER_TIMEOUT_0_TO_LOW_MS + 224 +
				     CONFIG_STEPPER_TEST_TIMEOUT_TOLERANCE_MS));
	STEPPER_POSITION_COMPARE_POSITIVE(fixture, target_position);
}
...
{
	int target_position =
		-(STEPPER_STEPS_0_TO_LOW + STEPPER_STEPS_LOW_TO_HIGH_LOWER_HALF + 1500);
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_HIGH_SPEED_INTERVAL));
	zassert_ok(stepper_run(fixture->dev, STEPPER_DIRECTION_NEGATIVE));
	k_msleep(STEPPER_TIMEOUT_0_TO_LOW_MS + STEPPER_TIMEOUT_LOW_TO_HIGH_HALF_MS);
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_LOW_SPEED_INTERVAL));
...
ZTEST_F(accel_controller, test_move_by_deceleration_to_acceleration_positive_direction)
{
	...
	POLL_AND_CHECK_SIGNAL(stepper_signal, stepper_event, STEPPER_EVENT_STEPS_COMPLETED,
			      K_MSEC(STEPPER_TIMEOUT_LOW_TO_HIGH_HALF_MS +
				     ...
				     580 + CONFIG_STEPPER_TEST_TIMEOUT_TOLERANCE_MS));
...
ZTEST_F(accel_controller, test_move_by_deceleration_to_deceleration_positive_direction)
{
	...
	k_msleep(STEPPER_TIMEOUT_LOW_TO_HIGH_MS);
	zassert_ok(stepper_set_microstep_interval(fixture->dev, STEPPER_LOW_SPEED_INTERVAL * 2));
	zassert_ok(stepper_move_by(fixture->dev, 825));
	/* Wait time is calculaed based on the original acceleration parameters and might need to be
	 * adapted for other ones
	 */
	POLL_AND_CHECK_SIGNAL(stepper_signal, stepper_event, STEPPER_EVENT_STEPS_COMPLETED,
			      K_MSEC(STEPPER_TIMEOUT_0_TO_LOW_MS + 100 +
				     CONFIG_STEPPER_TEST_TIMEOUT_TOLERANCE_MS));
